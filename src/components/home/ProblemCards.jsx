  ...
  return (
    <section className="bg-gb-charcoal py-20 sm:py-24 lg:py-28">
      <Container>
        ...
        <ul className="gb-stagger mt-12 grid grid-cols-1 border-t border-l border-gb-line sm:grid-cols-2 lg:mt-16 lg:grid-cols-4">
          {STARTING_POINTS.map((item) => (
            <Reveal as="li" key={item.question} variant="fade" className="border-r border-b border-gb-line">
              <Link
                to={item.to}
                className="group flex h-full flex-col gap-4 bg-gb-graphite p-6 transition-colors duration-500 hover:bg-gb-slate lg:p-7"
              >
                <span className="flex h-11 w-11 items-center justify-center border border-gb-line text-gb-gold transition-[border-color,color,transform] duration-500 ease-[var(--ease-gb)] group-hover:-translate-y-0.5 group-hover:border-gb-gold group-hover:text-gb-gold">
                  <Icon name={item.icon} className="h-5 w-5" />
                </span>

                <h3 className="text-[1.0625rem] leading-snug font-bold tracking-tight text-gb-silver-light transition-colors duration-500 group-hover:text-gb-silver-light sm:text-lg">
                  {item.question}
                </h3>

                <p className="text-[0.875rem] leading-relaxed text-gb-silver-dark transition-colors duration-500 group-hover:text-gb-silver">
                  {item.answer}
                </p>

                <span className="mt-auto flex items-center gap-2 pt-4 text-[0.75rem] font-semibold tracking-[0.08em] text-gb-silver uppercase transition-colors duration-500 group-hover:text-gb-gold">
                  {item.cta}
