      ...
      <Container>
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
          <div className="flex flex-col gap-7 lg:col-span-6 lg:pt-2">
            ...
            <Reveal>
              <h2 className="text-display text-gb-silver-light">Find space without the guesswork</h2>
            </Reveal>
            ...
            <ul className="gb-stagger mt-2 grid gap-x-8 gap-y-7 sm:grid-cols-2">
              {pillars.map((pillar) => (
                <Reveal as="li" key={pillar.title} className="flex flex-col gap-2.5">
                  <span className="flex h-9 w-9 items-center justify-center border border-gb-line text-gb-gold">
                    <Icon name={pillar.icon} className="h-4.5 w-4.5" />
                  </span>
                  <h3 className="text-[0.9375rem] font-semibold text-gb-silver-light">{pillar.title}</h3>
                  <p className="text-[0.875rem] leading-relaxed text-gb-silver-dark">{pillar.body}</p>
                </Reveal>
              ))}
            </ul>
