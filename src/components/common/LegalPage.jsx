...
export function LegalPage({ title, description, path, updated, sections }) {
  return (
    <>
      ...
      <section className="bg-gb-white py-16 sm:py-20 lg:py-24">
        <Container size="narrow">
          <p className="text-meta uppercase text-gb-concrete">Last updated {updated}</p>

          <div className="mt-10 flex flex-col gap-10">
            {sections.map((section) => (
              <Reveal key={section.heading} variant="fade" className="flex flex-col gap-4">
                <h2 className="text-display-sm text-gb-graphite">{section.heading}</h2>
                {section.body.map((paragraph) => (
                  <p
                    key={paragraph.slice(0, 30)}
                    className="text-[0.9375rem] leading-relaxed text-gb-industrial"
                  >
                    {paragraph}
                  </p>
                ))}
                {section.list ? (
                  <ul className="flex list-disc flex-col gap-2 pl-5">
                    {section.list.map((item) => (
                      <li key={item} className="text-[0.9375rem] leading-relaxed text-gb-industrial">
                        {item}
                      </li>
                    ))}
                  </ul>
