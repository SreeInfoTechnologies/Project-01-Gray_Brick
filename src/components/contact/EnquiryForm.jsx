...
export function EnquiryForm({ defaults = {}, facility = null, submitLabel = 'Request a call' }) {
  ...
  if (status === 'success') {
    return (
      <div
        className="gb-ticks flex flex-col gap-5 rounded-gb-sm border border-gb-gold/50 bg-gb-pure p-7 sm:p-9"
        role="status"
      >
        <span className="flex h-12 w-12 items-center justify-center rounded-gb-sm border border-gb-gold/60 text-gb-gold-dark">
          <Icon name="check" className="h-5 w-5" />
        </span>

        <div className="flex flex-col gap-3">
          <h3 className="text-display-sm text-gb-graphite">Thanks. That is with our team</h3>
  ...
  return (
    <form onSubmit={onSubmit} noValidate className="flex flex-col gap-6">
      {facility ? (
        <p className="flex items-center gap-2 rounded-gb-sm border border-gb-line-light bg-gb-white px-4 py-3 text-[0.8125rem] text-gb-industrial">
          <Icon name="warehouse" className="h-4 w-4 shrink-0 text-gb-gold-dark" />
          <span>
            About <span className="font-semibold text-gb-graphite">{facility}</span>
          </span>
        </p>
      ) : null}
      ...
      <div className={cn('flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between')}>
        ...
        <p className="text-[0.75rem] leading-relaxed text-gb-concrete sm:max-w-xs sm:text-right">
